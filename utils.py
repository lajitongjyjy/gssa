...
def compute_dist_matrix(dic, dataset, vocab_size=50000, data_dir="./"):
    ...
    dist_norms = np.linalg.norm(dist, axis=1, keepdims=True)
    dist_norms[dist_norms == 0] = 1
    dist = dist / dist_norms

    dist[0, :] = INFINITY
    dist[:, 0] = INFINITY
    dist[missed, :] = INFINITY
    dist[:, missed] = INFINITY
    dist_save_path = os.path.join(data_dir, "aux_files", "euclidean_dist_%s_%d.npy" % (dataset, vocab_size))
    np.save(dist_save_path, dist)
    print(f"Distance matrix saved to {dist_save_path}")
    AB = np.dot(embedding_matrix.T, embedding_matrix);
    norms = np.linalg.norm(AB, axis=1, keepdims=True)
    norms[norms == 0] = 1
    dist_cos = AB / norms
    rows, cols = dist_cos.shape
    #dist = dist + 0 * (np.ones((rows, cols)) - dist_cos);
    dist += 0 * (np.ones((rows, cols), dtype=np.float16) - dist_cos)
    dist = 0.5*dist + 0.5*dist_cos;
