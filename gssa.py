...
def gssa(
    ...
    query = eval(model)
    score_cache = tf.TensorArray(dtype=tf.float32, size=word_max_len, dynamic_size=False)
    def cond(adv_xs, modified_mask, synonyms, i, sorted_indices):
        return tf.less(i, max_iter)
    def body(adv_xs, modified_mask, synonyms, i, sorted_indices):
        ...
        for pos in tf.range(word_max_len):
            ...
            if tf.reduce_any(success_mask):
                return adv_xs, modified_mask, synonyms, max_iter, sorted_scores

        return adv_xs, modified_mask, synonyms, i + 1, sorted_scores
    x_adv, _, _, _, _ = tf.while_loop(
        cond,
        body,
        [adv_xs, modified_mask, synonyms, 0, tf.zeros([batch_size, word_max_len], tf.int32)],
        parallel_iterations=1
    )
    _, _, predictions, _ = query(x_adv, 1.0, dataset, reuse=True)
    suc_index = tf.not_equal(predictions, ys)
    return x_adv, suc_index, predictions
